...
class PacketStartReader extends Reader[PacketStartMarker] {
  val MARKER_SIZE = 4
  ...
  def read(marker: PacketStartMarker, packet: Packet): PacketStartMarker =
    PacketStartMarker(packet +: marker.packets.take(MARKER_SIZE - 1), marker.position + 1, marker.dupLocation).pipe(processFullMarker)
